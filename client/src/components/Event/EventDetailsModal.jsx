...
const EventDetailsModal = ({ isOpen, onClose, event, onEventDeleted }) => {
  ...
  const isCreator = currentUser?._id === event.creator._id;
  ...
  const handleDelete = async () => {
    try {
      ...
      const response = await fetch(`${url}/api/events/${event._id}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleRegister = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${url}/api/events/${event._id}/register`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleUnRegister = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${url}/api/events/${event._id}/unregister`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (data.success) {
        ...
        event.registrations = event.registrations.filter((id) => id !== currentUser._id);
        event.currentAttendees -= 1;
        onClose();
      } else {
        toast.error(data.message || 'Failed to unregister from the event');
      }
    } catch (error) {
